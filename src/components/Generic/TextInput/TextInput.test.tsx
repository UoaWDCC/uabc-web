...
describe('<TextInput />', () => {
  ...
  test('renders with label', () => {
    const { getByText } = render(<TextInput label="Username" type="text" />)
  ...
  test('displays error message when invalid', () => {
    const { getByText } = render(
      <TextInput type="text" label="Email" isError={true} errorMessage="Invalid email format" />,
    )
  ...
  test('handles disabled state', () => {
    const { getByPlaceholderText } = render(
      <TextInput type="text" disabled placeholder="Disabled input" />,
    )
  ...
  test('forwards ref correctly', () => {
    ...
    render(<TextInput type="text" ref={ref} />)
    expect(ref.current).toBeInstanceOf(HTMLInputElement)
  })

  test('applies custom styles and props', () => {
    const { getByTestId } = render(
      <TextInput type="text" data-testid="custom-input" width="300px" placeholder="Custom input" />,
    )
  ...
  test('handles input value changes', () => {
    const { getByPlaceholderText } = render(<TextInput type="text" placeholder="Enter text" />)
