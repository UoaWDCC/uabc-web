...
export const CreateSemesterRequestBody = z.object({
  ...
  breakStart: z.string().refine((val) => !Number.isNaN(Date.parse(val)), {
    message: "Invalid date format, should be in ISO 8601 format",
  }),
  breakEnd: z.string().refine((val) => !Number.isNaN(Date.parse(val)), {
    message: "Invalid date format, should be in ISO 8601 format",
  }),
  bookingOpenDay: z.nativeEnum(Weekday),
  bookingOpenTime: z.string().refine((val) => !Number.isNaN(Date.parse(val)), {
    message: "Invalid time format, should be in ISO 8601 format",
  }),
})
