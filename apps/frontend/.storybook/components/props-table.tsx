import { Center, Flex, For, Grid, GridItem, Heading, toTitleCase } from "@yamada-ui/react"
import type { ReactNode } from "react"
import { Fragment } from "react"

interface PropsTableProps<Y extends string, M extends string> {
  children: (column: Y, row: M, key: string, colIndex: number, rowIndex: number) => ReactNode
  rows: M[]
  variant?: "column" | "grid"
  columns?: Y[]
}
        ...
        <For each={rows}>
          {(row, rowIndex) => (
            <Fragment key={rowIndex}>
              <Flex align="center" color="fg.emphasized" fontWeight="medium" p="md">
                {toTitleCase(row)}
              </Flex>
              ...
              <For each={columns}>
                {(column, colIndex) => (
                  <GridItem
                    ...
                    roundedTopRight={
                      colIndex === columns.length - 1 && !rowIndex ? "l3" : undefined
                    }
                  >
                    {children(column, row, `${colIndex}-${rowIndex}`, colIndex, rowIndex)}
                  </GridItem>
                )}
              </For>
            </Fragment>
        ...
        {(row, rowIndex) => (
          <Fragment key={rowIndex}>
            ...
            <GridItem
              ...
              p="md"
            >
              {children(undefined as unknown as Y, row, rowIndex.toString(), 0, rowIndex)}
            </GridItem>
          </Fragment>
        )}
      </For>
